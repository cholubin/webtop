...
class MytemplatesController < ApplicationController
  ...
  def index
    ...
    if cate == "all" and folder == "all"
      ...
      @mytemplates = Mytemplate.all(:type => cate, :user_id => current_user.id, :order => [:created_at.desc]).search(params[:search], params[:page])                                   
    elsif cate != "all" and folder != "all"
      @mytemplates = Mytemplate.all(:folder => Tempfolder.get(folder).name, :type => cate, :user_id => current_user.id, :order => [:created_at.desc]).search(params[:search], params[:page])                           
    end
